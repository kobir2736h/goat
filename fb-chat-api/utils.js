...
// Create an Axios client wrapped with cookiejar support
let client = wrapper(axios.create({ withCredentials: true }));
let proxyConfig = false;

/**
 * @param {string} urlStr
 */
function setProxy(urlStr) {
    ...
    client = wrapper(axios.create({ withCredentials: true, proxy: proxyConfig }));
}
...
async function get(url, jar, qs, options, ctx) {
    ...
    const config = {
        ...
        jar: jar,
        timeout: 60000,
        responseType: 'text' // Important to read raw HTML/JSON like request did
    };
    ...
    try {
        ...
        response.body = response.data; // For backward compatibility
...
async function post(url, jar, form, options, ctx, customHeader) {
    ...
    if (proxyConfig) config.proxy = proxyConfig;
    const bodyData = querystring.stringify(form);

    try {
        const response = await client.post(url, bodyData, config);
...
function presenceEncode(str) {
    return encodeURIComponent(str)
        .replace(/([_A-Z])|%../g, function (m, n) { return n ? "%" + n.charCodeAt(0).toString(16) : m; })
        .toLowerCase()
        .replace(h, function (m) { return i[m]; });
}

function presenceDecode(str) {
    return decodeURIComponent(str.replace(/[_A-Z]/g, function (m) { return j[m]; }));
}
    ...
    return (
        ...
        presenceEncode(
            JSON.stringify({
                ...
                user: userID,
                state: { ut: 0, t2: [], lm2: null, uct2: time, tr: null, tw: Math.floor(Math.random() * 4294967295) + 1, at: time },
                ch: { ["p_" + userID]: 0 }
            })
        )
    );
...
// All formatting functions (_formatAttachment, formatDeltaMessage, etc.) remain unchanged
function _formatAttachment(attachment1, attachment2) {
    ...
    switch (type) {
        ...
        case "MessageVideo": return { type: "video", filename: blob.filename, ID: blob.legacy_attachment_id, previewUrl: blob.large_image.uri, previewWidth: blob.large_image.width, previewHeight: blob.large_image.height, url: blob.playable_url, width: blob.original_dimensions.x, height: blob.original_dimensions.y, duration: blob.playable_duration_in_ms, videoType: blob.video_type.toLowerCase(), thumbnailUrl: blob.large_image.uri };
        case "MessageAudio": return { type: "audio", filename: blob.filename, ID: blob.url_shimhash, audioType: blob.audio_type, duration: blob.playable_duration_in_ms, url: blob.playable_url, isVoiceMail: blob.is_voicemail };
        case "StickerAttachment": return { type: "sticker", ID: blob.id, url: blob.url, packID: blob.pack ? blob.pack.id : null, spriteUrl: blob.sprite_image, spriteUrl2x: blob.sprite_image_2x, width: blob.width, height: blob.height, caption: blob.label, description: blob.label, frameCount: blob.frame_count, frameRate: blob.frame_rate, framesPerRow: blob.frames_per_row, framesPerCol: blob.frames_per_column, stickerID: blob.id, spriteURI: blob.sprite_image, spriteURI2x: blob.sprite_image_2x };
        case "MessageLocation":
            var u = querystring.parse(url.parse(blob.story_attachment.url).query).u;
            var where1 = querystring.parse(url.parse(u).query).where1;
            var address = where1.split(", ");
            var latitude, longitude;
            try { latitude = Number.parseFloat(address[0]); longitude = Number.parseFloat(address[1]); } catch (err) { }
            return { type: "location", ID: blob.legacy_attachment_id, latitude: latitude, longitude: longitude, image: blob.story_attachment.media && blob.story_attachment.media.image ? blob.story_attachment.media.image.uri : null, width: blob.story_attachment.media && blob.story_attachment.media.image ? blob.story_attachment.media.image.width : null, height: blob.story_attachment.media && blob.story_attachment.media.image ? blob.story_attachment.media.image.height : null, url: u || blob.story_attachment.url, address: where1, facebookUrl: blob.story_attachment.url, target: blob.story_attachment.target, styleList: blob.story_attachment.style_list };
        case "ExtensibleAttachment": return { type: "share", ID: blob.legacy_attachment_id, url: blob.story_attachment.url, title: blob.story_attachment.title_with_entities.text, description: blob.story_attachment.description && blob.story_attachment.description.text, source: blob.story_attachment.source ? blob.story_attachment.source.text : null, image: blob.story_attachment.media && blob.story_attachment.media.image && blob.story_attachment.media.image.uri, width: blob.story_attachment.media && blob.story_attachment.media.image && blob.story_attachment.media.image.width, height: blob.story_attachment.media && blob.story_attachment.media.image && blob.story_attachment.media.image.height, playable: blob.story_attachment.media && blob.story_attachment.media.is_playable, duration: blob.story_attachment.media && blob.story_attachment.media.playable_duration_in_ms, playableUrl: blob.story_attachment.media == null ? null : blob.story_attachment.media.playable_url, subattachments: blob.story_attachment.subattachments, properties: blob.story_attachment.properties.reduce(function (obj, cur) { obj[cur.key] = cur.value.text; return obj; }, {}), facebookUrl: blob.story_attachment.url, target: blob.story_attachment.target, styleList: blob.story_attachment.style_list };
        case "MessageFile": return { type: "file", filename: blob.filename, ID: blob.message_file_fbid, url: blob.url, isMalicious: blob.is_malicious, contentType: blob.content_type, name: blob.filename, mimeType: "", fileSize: -1 };
        default: throw new Error("unrecognized attach_file of type " + type + "`" + JSON.stringify(attachment1, null, 4) + " attachment2: " + JSON.stringify(attachment2, null, 4) + "`");
    }
}
...
function formatDeltaMessage(m) {
    ...
    return { type: "message", senderID: formatID(md.actorFbId.toString()), threadID: formatID((md.threadKey.threadFbId || md.threadKey.otherUserFbId).toString()), messageID: md.messageId, args: args, body: body, attachments: (m.delta.attachments || []).map(v => _formatAttachment(v)), mentions: mentions, timestamp: md.timestamp, isGroup: !!md.threadKey.threadFbId, participantIDs: m.delta.participants || (md.cid ? md.cid.canonicalParticipantFbids : []) || [] };
}
...
function formatHistoryMessage(m) {
    switch (m.action_type) {
        case "ma-type:log-message": return formatEvent(m);
...
function getAdminTextMessageType(m) {
    switch (m.type) {
        ...
        case "pin_messages_v2": return "log:thread-pinned";
    }
}

function getGenderByPhysicalMethod(name) {
    const female_name = ["sumaiya", "rahima", "munni", "trisha", "mim", "sumona", "asha", "trisha", "suraiya"];
    let OtherName = [".", ",", "/", "%", "&", "*", "-", "+"];
    try {
        var Name;
        if (name == " " || name == null) return "UNKNOWN";
        if (female_name.some(a => name.includes(a))) {
            if (!OtherName.includes(name)) Name = "FEMALE";
            else Name = ['FEMALE', 'MALE'][Math.floor(Math.random() * 2)];
        } else {
...
function arrayToObject(arr, getKey, getValue) {
    return arr.reduce(function (acc, val) { acc[getKey(val)] = getValue(val); return acc; }, {});
}
...
function parseAndCheckLogin(ctx, defaultFuncs, retryCount = 0) {
    return async function (data) {
        // Here `data` is the Axios response object
        try {
            if (data.statusCode >= 500 && data.statusCode < 600) {
                if (retryCount >= 5) {
                    throw { error: "Request retry failed. Check the `res` and `statusCode` property on this error.", statusCode: data.statusCode, res: data.body };
                }
                retryCount++;
                var retryTime = Math.floor(Math.random() * 5000);
                console.warn(`[WARN]: Got status code ${data.statusCode} - ${retryCount}. attempt to retry in ${retryTime} milliseconds...`);
                
                await sleep(retryTime);
                const reqUrl = data.config.url;
                
                if (data.config.headers && data.config.headers["Content-Type"] && data.config.headers["Content-Type"].includes("multipart/form-data")) {
                    const retryRes = await defaultFuncs.postFormData(reqUrl, ctx.jar, JSON.parse(data.config.data), {});
                    return parseAndCheckLogin(ctx, defaultFuncs, retryCount)(retryRes);
                } else {
                    const retryRes = await defaultFuncs.post(reqUrl, ctx.jar, querystring.parse(data.config.data));
                    return parseAndCheckLogin(ctx, defaultFuncs, retryCount)(retryRes);
                }
            }
            ...
            try {
                ...
                throw { error: "JSON.parse error. Check the `detail` property on this error.", detail: e, res: data.body };
            }

            if (res.redirect && data.config.method === "get") {
                const redirectRes = await defaultFuncs.get(res.redirect, ctx.jar);
                return parseAndCheckLogin(ctx, defaultFuncs)(redirectRes);
            }
...
function saveCookies(jar) {
    return function (res) {
        var cookies = res.headers ? (res.headers["set-cookie"] || []) : [];
...
function getAppState(jar) {
    return jar.getCookiesSync("https://www.facebook.com").concat(jar.getCookiesSync("https://facebook.com")).concat(jar.getCookiesSync("https://www.messenger.com"));
}

function getData_Path(Obj, Arr, Stt) {
    if (Arr.length === 0 && Obj != undefined) return Obj;
    else if (Obj == undefined) return Stt;
    const head = Arr[0];
    if (head == undefined) return Stt;
...
function setData_Path(obj, path, value) {
    ...
    if (!currentObj) { obj[currentKey] = value; currentObj = obj[currentKey]; }
    path.shift();
    if (!path.length) currentObj = value;
    else currentObj = setData_Path(currentObj, path, value);
    return obj;
}
...
function getPaths(obj, parentPath = []) {
    let paths = [];
    for (let prop in obj) {
        if (typeof obj[prop] === "object") paths = paths.concat(getPaths(obj[prop], [...parentPath, prop]));
        else paths.push([...parentPath, prop]);
    }
    return paths;
}
...
function cleanHTML(text) {
    text = text.replace(/(<br>)|(<\/?i>)|(<\/?em>)|(<\/?b>)|(!?~)|(&amp;)|(&#039;)|(&lt;)|(&gt;)|(&quot;)/g, (match) => {
        switch (match) {
            case "<br>": return "\n";
            case "<i>": case "<em>": case "</i>": case "</em>": return "*";
