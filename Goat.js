...
if (config.autoRestart) {
	...
	if (!isNaN(time) && time > 0) {
		utils.log.info("AUTO RESTART", `Bot will automatically restart after ${utils.convertTime(time, true)}`);
		setTimeout(() => {
		...
		utils.log.info("AUTO RESTART", `Bot will automatically restart at cron time: ${time}`);
...
(async () => {
	...
	try {
		...
		throw new Error("Google API token has expired or is invalid. Please get a new token.");
	}
	...
	if (compareVersion(version, currentVersion) === 1)
		utils.log.master("NEW VERSION", `New version detected: ${colors.hex("#eb6a07", version)} (current: ${colors.gray(currentVersion)}). Please run "node update"`);
